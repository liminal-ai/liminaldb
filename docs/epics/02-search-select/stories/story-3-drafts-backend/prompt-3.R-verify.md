...
- [ ] Draft keys and IDs follow `edit:{slug}` or `new:{uuid}`
...
- [ ] Draft API tests authenticate using test auth helpers (e.g., `getTestAuth` / `requireTestAuth`) or mock `getAuthenticatedUser`
